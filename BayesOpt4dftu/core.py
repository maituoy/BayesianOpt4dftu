...
class delta_band(object):
    ...
    def deltaBand(self):
        ...
        if nbands_hse != nbands_dftu:
            raise Exception(
                'Error: The band number of HSE and GGA+U are not match!')
        ...
        if nkpts_hse - kpts_diff != nkpts_dftu:
            raise Exception(
                'Error: The kpoints number of HSE and GGA+U are not match!')
        ...
        if ispin_hse == 1 and ispin_dftu == 1:
            ...
            raise Exception(
                'Error: The spin number of HSE and GGA+U are not match!')
        ...
        if ispin_hse == 2:
            shifted_hse = np.concatenate((shifted_hse,
                                          (v['hse'][1] - edge['hse'][0][0])[:, loc['hse']
                                                                            [1][0]-self.br+1:loc['hse'][1][0]+1],
                                          (v['hse'][1] - edge['hse'][0][1])[:, loc['hse'][1][1]:loc['hse'][1][1]+self.br]),
                                         axis=1)
        if ispin_dftu == 1:
            continuous = (loc['dftu'][0][1] - loc['dftu'][0][0]) == 1
        elif ispin_dftu == 2:
            continuous = (loc['dftu'][0][1] - loc['dftu'][0][0]
                          ) == 1 & (loc['dftu'][1][1] - loc['dftu'][1][0]) == 1
        else:
        ...
        if bs_dftu.is_metal() == False or continuous == True:
            ...
            if ispin_dftu == 2:
                shifted_dftu = np.concatenate((shifted_dftu,
                                               (v['dftu'][1] - edge['dftu'][0][0])[:,
                                                                                   loc['dftu'][1][0]-self.br+1:loc['dftu'][1][0]+1],
                                               (v['dftu'][1] - edge['dftu'][0][1])[:, loc['dftu'][1][1]:loc['dftu'][1][1]+self.br]),
                                              axis=1)
        else:
            shifted_dftu = (v['dftu'][0] - edge['dftu'][0][0])[:,
                                                               loc['dftu'][0][0]-self.br+1:loc['dftu'][0][0]+1+self.br]
            if ispin_dftu == 2:
                shifted_dftu = np.concatenate((shifted_dftu,
    ...
    def bo(self, num_variables):
        ...
        with open('input.json', 'r') as f:
            ...
            for i in range(num_variables):
                data["general_flags"]["ldau_luj"][elements[i]
                                                  ]["U"] = float(U[i])
            f.close()
